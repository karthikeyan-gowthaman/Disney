   ...
   <xsl:template match="/">
      <Inbox>
         ...
         <xsl:attribute name="ExceptionType">
            <xsl:value-of select="'Shipment Delayed'" />
         </xsl:attribute>
          <xsl:attribute name="QueueId">
            <xsl:value-of select="'ShipmentDelayedQueue'" />
         </xsl:attribute>
         <xsl:attribute name="Description">
            <xsl:value-of select="'Sales Order Stuck in Release Status from last 2 or more days'" />
         </xsl:attribute>
